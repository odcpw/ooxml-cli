...
fn xlsx_forms_entry_creates_macro_workbook_with_non_activex_button() {
    ...
    assert!(form_sheet.contains(r#"<c r="A3" t="inlineStr"><is><t>Name</t></is></c>"#));
    assert!(form_sheet.contains(r#"<c r="A5" t="inlineStr"><is><t>Notes</t></is></c>"#));

    let data_sheet = read_zip_string(&out, "xl/worksheets/sheet2.xml");
    assert!(data_sheet.contains(r#"<c r="A1" t="inlineStr"><is><t>Timestamp</t></is></c>"#));
    assert!(data_sheet.contains(r#"<c r="D1" t="inlineStr"><is><t>Notes</t></is></c>"#));
    ...
    assert!(source.contains("dataSheet.Cells(nextRow, 4).Value = formSheet.Range(\"B5\").Value"));
