...
use crate::{
    ...
    command_arg, content_type_for_part, copy_zip_with_binary_part_overrides_and_removals,
...
fn resolve_media_kind(explicit: Option<&str>, ext: &str) -> CliResult<String> {
...
fn content_type_for_media_ext(ext: &str) -> String {
    match ext.trim_start_matches('.').to_ascii_lowercase().as_str() {
        "mp4" | "m4v" => "video/mp4",
        "mov" => "video/quicktime",
        "avi" => "video/x-msvideo",
...
fn poster_content_type_for_path(path: &str) -> String {
    match Path::new(path)
        .extension()
...
fn file_extension_with_dot(path: &str) -> String {
...
fn reject_media_url(path: &str) -> CliResult<()> {
