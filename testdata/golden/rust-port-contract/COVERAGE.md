...
| PPTX template/layout QA read-only | Go-vs-Rust differential tests for `pptx template inspect` and `pptx validate-layout`, including manifest inspection, empty/moderate/dense slide density, text overflow, shape collision, missing files, and non-PPTX rejection | `tests/rust_contract_smoke/pptx.rs` `pptx_template_inspect_and_validate_layout_match_go_oracle` |
| PPTX chart leaf commands | Go-vs-Rust differential tests for `pptx charts list/show`, `create`, and `update-data`, saved-output readback/strict validation for chart style edits, `set-axis`, `convert-type`, and `copy-style`, plus representative error envelopes | `cargo test --test rust_contract_smoke pptx_charts -- --nocapture` |
...
| DOCX replace | Go-vs-Rust differential tests for `docx replace` dry-run, saved-output readback, strict validation, split-run/table-cell replacement, regex, whole-word, and representative error behavior | `tests/rust_contract_smoke/docx.rs` `docx_replace_*` |
