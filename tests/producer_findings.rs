...
#[test]
fn libreoffice_docx_justification_is_reported_only_in_invalid_contexts() {
    let file =
        Path::new(env!("CARGO_MANIFEST_DIR")).join("testdata/corpus/libreoffice/report.docx");
    let original = fs::read(&file).unwrap();
    let output = Command::new(env!("CARGO_BIN_EXE_ooxml"))
        .args([
            "--json",
    ...
    let findings: Vec<_> = report["findings"]
        ...
        .unwrap()
        .iter()
        .filter(|finding| finding["code"] == "DOCX_JUSTIFICATION_PRODUCER_VALUE")
        .collect();
    assert_eq!(findings.len(), 64, "{report}");
    for (part, context, count) in [
        ("/word/document.xml", "table", 1),
        ("/word/numbering.xml", "numbering level", 63),
    ] {
        let matched: Vec<_> = findings
            .iter()
            .filter(|finding| finding["part"] == part)
            .collect();
        assert_eq!(matched.len(), count);
        for finding in matched {
            assert_eq!(finding["severity"], "error");
            assert_eq!(finding["location"]["context"], context);
            assert_eq!(finding["location"]["value"], "start");
            assert_eq!(finding["docs"], "docs/producer-limitations.md");
            assert!(
                finding["message"]
                    .as_str()
                    .unwrap()
                    .contains("writing direction")
            );
        }
    }
    assert_eq!(fs::read(file).unwrap(), original);
