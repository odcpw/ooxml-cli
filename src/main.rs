...
fn capabilities(args: &[String]) -> CliResult<Value> {
    ...
    Ok(json!({
        ...
        "objectKindsIndex": {
            ...
            "slide": ["ooxml pptx slides list", "ooxml pptx slides selectors", "ooxml pptx slides show", "ooxml pptx replace text", "ooxml pptx render"],
            "shape": ["ooxml pptx slides list", "ooxml pptx slides selectors", "ooxml pptx slides show", "ooxml pptx replace text"],
...
fn pptx_selector_targets(xml: &str) -> Vec<Value> {
    ...
    shapes
        ...
        .map(|(index, shape)| {
            ...
            target.insert("shapeType".to_string(), json!("sp"));
            target.insert(
                "targetKind".to_string(),
                json!(if !placeholder_role.is_empty() {
                    placeholder_role
            ...
            target.insert("textCapable".to_string(), json!(shape.has_text_body));
...
fn pptx_shape_models(xml: &str) -> Vec<Shape> {
    ...
    loop {
        match reader.read_event() {
            ...
            Ok(Event::Start(e)) | Ok(Event::Empty(e))
                if current.is_some() && local_name(e.name().as_ref()) == "txBody" =>
            {
                if let Some(shape) = current.as_mut() {
